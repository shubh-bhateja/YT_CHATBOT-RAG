...
def process_transcript(raw_transcript, meta, target_chunk_chars=600):
    ...
    # --- Doc #0: Video Identity (solves "who are the speakers?" questions) ---
            ...
            with st.status("🚀 Initializing AI Engine...", expanded=True) as status:
                ...
                if not raw_transcript:
                    ...
                    try:
                        ...
                        if audio_path:
                            audio_utils.cleanup_audio(audio_path)
                ...
                if raw_transcript:
                    ...
                    # --- NO MultiQueryRetriever → saves 3+ LLM calls per question ---
                    # PDR handles semantic search, BM25 handles keyword search
        ...
        if prompt := st.chat_input("Ask anything about this video..."):
            if not st.session_state.retriever:
                ...
                with st.chat_message("assistant"):
                    # --- Retrieval (ZERO LLM calls — purely embedding + keyword) ---
                    ...
                    if is_blocked:
                        ...
                        # --- Get working LLM (Gemini → Mistral fallback) ---
...
# Initialize static-ffmpeg to ensure it's in the path
static_ffmpeg.add_paths()

def download_audio(url, output_dir="temp_audio"):
    """
    Download audio using pytubefix - bypasses YouTube bot detection 
    via PoToken signing, no cookies or authentication required.
    Works on any device: laptop, phone, server, etc.
    """
...
def transcribe_audio(audio_path, model_size="base"):
    ...
    Runs on CPU (int8) for maximum device compatibility.
    """
...
def generate_synthetic_data(transcript_docs, llm, num_questions=3):
    ...
    # Use the first 3 chunks as sample context
...
def run_evaluation(video_id="dQw4w9WgXcQ"): # Default to RickRoll for testing if none provided
    """
    Runs a REAL RAGAS evaluation on a given YouTube video.
    """
    print(f"🚀 Starting Real RAG Evaluation for video: {video_id}...")
    ...
    # 2. Setup Evaluator LLM
    llm = HuggingFaceEndpoint(repo_id='mistralai/Mistral-7B-Instruct-v0.2', task='text-generation', temperature=0.1)
    evaluator_llm = ChatHuggingFace(llm=llm)

    # 3. Generate Questions & Ground Truth
    ...
    # 4. Mocking RAG retrieval for this script (In a real app, this would call your RAG pipeline)
    # We will "pretend" to retrieve the first 2 docs for each question for the sake of the evaluation demo
    sample_contexts = [[docs[0].page_content, docs[1].page_content]] * len(questions)
    
    # 5. Generate actual answers from the RAG system (Simplified here)
    sample_answers = []
    for q in questions:
        resp = evaluator_llm.invoke(f"Answer the question based on context: {q}\n\nContext: {docs[0].page_content}")
    ...
    try:
        ...
        print("Note: Ragas often requires high-tier models (GPT-4) for stable metric calculation.")
